...
class GraphManager:
    ...
    """
    def __init__(self, init_graph_json: str | None = None, start_region: str = "region_0", start_coords: tuple[float, float] = (0.0, 0.0)) -> None:
        self.graph = GraphHandler(init_graph_json or "", init_node=start_region)
        self.updater = UpdatePromptFormer()
        self.current_region = start_region

        if not self.graph.graph.has_node(start_region):
    ...
    def update_pose(self, xy: Sequence[float]) -> None:
        ...
        """
        self.graph.update_location(self.current_region)
    ...
    def add_object_detection(self, label: str, xy: Sequence[float]) -> None:
        ...
        self.graph.update_with_node(node=node_id, edges=[self.current_region], attrs=attrs)
        ...
        with open(os.path.join(log_dir, "graph_history.jsonl"), "a") as f:
            json.dump({
                "event": "add_object_detection",
                "node": node_id,
                "coords": list(map(float, xy)),
                "current_region": self.current_region,
                "graph": self.graph.as_json_str,
            }, f)
            f.write("\n")  
    ...
    def add_region(self, region_xy: Sequence[float]) -> str:
        ...
        self.graph.update_with_node(node=name, edges=[self.current_region], attrs=attrs)
